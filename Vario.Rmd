...
v_df %>% as.data.frame %>% 
  ...
  ggtitle("Wind Speed (m/s) from hurr_tracks") + coord_equal() + theme_bw()
coordinates(v_df)= ~ latitude+longitude
#Histgram of wind
hist(v_df$wind)
```
...
```{r}
#by.wind <- plyr::ddply(by.map,c("buoy"),numcolwise(mean)) ???????????????????
...
WSPD.vgm <- variogram(log(WSPD)~1, by.map) # calculates sample variogram values
plot(WSPD.vgm)
WSPD.vgm.model <- vgm( nugget = min(WSPD.vgm$gamma), 
                       model="Gau", 
                       psill=max(WSPD.vgm$gamma), 
                       range = 2) # range of dist

#"fit" out variogram model to the variogram.
FittedModel2 <- fit.variogram(WSPD.vgm, model=WSPD.vgm.model, fit.method = 2)    
plot(WSPD.vgm, model=FittedModel2, main="Variogram of Wind Speed of Irene2011 \n based on Buoys")
```
...
by.map <- na.omit(by.map)
by.wind <- plyr::ddply(by.map,c("buoy"),numcolwise(mean))
by.wind %>% as.data.frame %>% 
  ggplot(aes(lat, lon)) + geom_point(aes(size=WSPD), color="purple", alpha=3/4) + 
  ggtitle("Wind Speed (m/s) from Buoys") + coord_equal() + theme_bw()
coordinates(by.wind)= ~ lat+lon
#Histgram of wind
hist(by.wind$WSPD)


WSPD.vgm2 <- variogram(log(WSPD)~1, by.wind) # calculates sample variogram values
plot(WSPD.vgm2)
...
#"fit" out variogram model to the variogram.
FittedModel3 <- fit.variogram(WSPD.vgm2, model=WSPD.vgm.model2)    
