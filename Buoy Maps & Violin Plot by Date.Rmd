...
df1 <- mr41037 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "41037",lat = 33.988,lon = -77.362,
                                                     Code="NC",State="North Carolina")
df2 <- mr41110 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "41110",lat = 34.142,lon = -75.454)
df3 <- mr41025 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "41025",lat = 35.010,lon = -77.715)
df4 <- mr44056 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "44056",lat = 36.200,lon = -75.714)
df5 <- mrbftn7 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "bftn7",lat = 34.717,lon = -76.671,
                                                     Code="NC",State="North Carolina")
df6 <- mrclkn7 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "clkn7",lat = 34.622,lon = -76.525,
                                                     Code="NC",State="North Carolina")
df7 <- mrjmpn7 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "jmpn7",lat = 34.213,lon = -77.786,
                                                     Code="NC",State="North Carolina")
df8 <- mrhcgn7 %>% select(DD, WSPD, ATMP, TIDE) %>% mutate(buoy = "hcgn7",lat = 35.209,lon = -75.704,
                                                     Code="NC",State="North Carolina")
df.row <- bind_rows(df1,df2,df3,df4,df5,df6,df7,df8)
df.row$WSPD <- as.numeric(df.row$WSPD)
df.row$ATMP <- as.numeric(df.row$ATMP)
df.row$TIDE <- as.numeric(df.row$TIDE)

by.map <- plyr::ddply(df.row,c("buoy", "DD"),numcolwise(mean))
