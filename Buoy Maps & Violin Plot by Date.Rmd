...
#df3 <- mr41025 %>% select(DD, WSPD, ATMP, TIDE, APD) %>% mutate(buoy = "41025",lat = 35.010,lon = -77.715)
...
df8 <- mrhcgn7 %>% select(DD, WSPD, ATMP, PRES) %>% mutate(buoy = "hcgn7",lat = 35.209,lon = -75.704,
                                                     Code="NC",State="North Carolina")
df.row <- bind_rows(df1,df5,df6,df7,df8)
...
by.map <- df.row %>% 
  ...
  group_by(date) %>%
  summarize(mean = mean(WSPD))

=======
df.row$PRES <- as.numeric(df.row$PRES)
by.map <- plyr::ddply(df.row,c("buoy", "DD"),numcolwise(mean))
>>>>>>> be8f7d165f2ccea8ce55a4dcd04df869b47cb8e9
